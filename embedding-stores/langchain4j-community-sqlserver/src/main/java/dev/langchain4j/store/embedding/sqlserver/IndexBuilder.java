...
/**
 * <p>
 *   Abstract class that contains common methods for IndexBuilders. SQL Server
 *   supports DiskANN vector indexes which can be implemented extending this class.
 * </p>
...
abstract class IndexBuilder<T extends IndexBuilder> {
    ...
    /**
     * Truncates the index name if it is longer than the maximum length allowed
     * by the database.
     ...
     */
    private static String truncateIndexName(String indexName, boolean isQuoted) {
     ...
     */
    private static String unquoteTableName(String tableName) {
    ...
    String getDropIndexStatement(EmbeddingTable embeddingTable) {
        return "DROP INDEX IF EXISTS " + getIndexName(embeddingTable) + " ON " + embeddingTable.getQualifiedTableName();
    }
