 ...
 * <ul>
 *   <li>{@code OCEANBASE_URI}: JDBC url (e.g., "jdbc:oceanbase://127.0.0.1:2881/test")</li>
...
class OceanBaseFilterIT {
    ...
    void test_filter_by_metadata_field() {
        ...
        embeddingStore.add(
                ...
                TextSegment.from(
                        "Machine learning", new Metadata().put("category", "ai").put("language", "Python")));
        ...
        assertThat(results.matches())
                .extracting(m -> m.embedded().metadata().toMap().get("category"))
    ...
    void test_filter_with_is_in() {
        ...
        assertThat(results.matches())
                .extracting(m -> m.embedded().metadata().toMap().get("language"))
    ...
    void test_filter_with_hybrid_search() {
        ...
        embeddingStore.add(
                ...
                TextSegment.from("Java programming language", Metadata.from("category", "programming")));

        embeddingStore.add(
                embeddingModel.embed("Python programming language").content(),
                TextSegment.from("Python programming language", Metadata.from("category", "programming")));

        embeddingStore.add(
                embeddingModel.embed("Machine learning algorithms").content(),
                TextSegment.from("Machine learning algorithms", Metadata.from("category", "ai")));
