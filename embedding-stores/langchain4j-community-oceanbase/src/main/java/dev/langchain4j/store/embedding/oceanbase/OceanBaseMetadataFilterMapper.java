...
class OceanBaseMetadataFilterMapper {
    ...
    private static String convertKey(String key, FieldDefinition fieldDefinition) {
        ...
        if (keyLower.equals(fieldDefinition.getIdFieldName().toLowerCase())) {
            return fieldDefinition.getIdFieldName();
                ...
                || keyLower.equals("document")
                || keyLower.equals("text")) {
            ...
            return fieldDefinition.getMetadataFieldName();
                ...
                || keyLower.equals("vector")
                || keyLower.equals("embedding")) {
            ...
            return format("JSON_UNQUOTE(JSON_EXTRACT(%s, '$.%s'))", fieldDefinition.getMetadataFieldName(), key);
        }
    }
    ...
    private static String mapNotEqual(IsNotEqualTo isNotEqualTo, FieldDefinition fieldDefinition) {
        ...
        if (keyExpr.contains("JSON_EXTRACT")) {
            ...
            return format(
                    "(%s != %s OR %s IS NULL)", keyExpr, formatValue(isNotEqualTo.comparisonValue()), jsonExtractExpr);
        }
    ...
    private static String mapIn(IsIn isIn, FieldDefinition fieldDefinition) {
        ...
        List<?> values =
                valuesCollection instanceof List ? (List<?>) valuesCollection : new ArrayList<>(valuesCollection);
        if (values.isEmpty()) {
    ...
    private static String mapNotIn(IsNotIn isNotIn, FieldDefinition fieldDefinition) {
        ...
        if (keyExpr.contains("JSON_EXTRACT")) {
            ...
            return format("(NOT (%s) OR %s IS NULL)", String.join(" OR ", conditions), jsonExtractExpr);
        }
    ...
    private static String mapNot(Not not, FieldDefinition fieldDefinition) {
        ...
        if (keyExpr != null && keyExpr.contains("JSON_EXTRACT")) {
            ...
            if (innerFilter instanceof IsEqualTo isEqualTo) {
                ...
                return format(
                        "(%s != %s OR %s IS NULL)", keyExpr, formatValue(isEqualTo.comparisonValue()), jsonExtractExpr);
                ...
                Collection<?> valuesCollection = isIn.comparisonValues();
                List<?> values = valuesCollection instanceof List
                        ? (List<?>) valuesCollection
                        : new ArrayList<>(valuesCollection);
                if (values.isEmpty()) {
                ...
                return format(
                        "(%s NOT IN (%s) OR %s IS NULL)", keyExpr, String.join(", ", valueStrings), jsonExtractExpr);
            } else {
