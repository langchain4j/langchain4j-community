...
public class VearchEmbeddingStore implements EmbeddingStore<TextSegment> {
    ...
    public EmbeddingSearchResult<TextSegment> search(EmbeddingSearchRequest request) {
        ...
        List<String> fields =
                new ArrayList<>(Arrays.asList(vearchConfig.getTextFieldName(), vearchConfig.getEmbeddingFieldName()));
        ...
        List<EmbeddingMatch<TextSegment>> matches =
                toEmbeddingMatch(response.getDocuments().get(0));
        ...
        return documents.stream()
                .map(document -> {
                    ...
                    Embedding embedding = Embedding.from(
                            vector.stream().map(Double::floatValue).collect(toList()));
                    ...
                    return new EmbeddingMatch<>(
                            RelevanceScore.fromCosineSimilarity(
                                    ((Number) document.get(DEFAULT_SCORE_FILED_NAME)).doubleValue()),
                            id,
                            embedding,
                            textSegment);
                })
