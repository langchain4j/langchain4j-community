 ...
 *
 * <p>Uses {@code GET /wiki/rest/api/content} (Confluence Cloud) with pagination
 * to fetch all pages.
 */
public class ConfluenceDocumentLoader {
     ...
     *
     * <p>Pagination is handled automatically (the Confluence API defaults to 25
     * results per page).
     */
    ...
    private JsonNode fetchContent(int start, int limit) {
        ...
        try {
            ...
            throw new ConfluenceDocumentLoaderException(
                    "Confluence API request failed with status code " + e.statusCode() + ": " + e.getMessage(), e);
        } catch (IOException e) {
            throw new ConfluenceDocumentLoaderException("Failed to call Confluence API: " + uri, e);
        } catch (RuntimeException e) {
            if (e.getCause() instanceof InterruptedException) {
                Thread.currentThread().interrupt();
                throw new ConfluenceDocumentLoaderException("Confluence API request interrupted: " + uri, e);
            }
            if (e.getCause() instanceof HttpException httpException) {
                throw new ConfluenceDocumentLoaderException(
                        "Confluence API request failed with status code "
    ...
    private Document toDocument(JsonNode page) {
        ...
        String title = page.path(METADATA_TITLE).asText(null);
