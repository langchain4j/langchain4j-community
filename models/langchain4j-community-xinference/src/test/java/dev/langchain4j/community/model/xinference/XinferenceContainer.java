    ...
    public XinferenceContainer(DockerImageName image) {
        ...
        if (runtimes != null && runtimes.containsKey("nvidia")) {
            this.withCreateContainerCmdModifier((cmd) -> {
                Objects.requireNonNull(cmd.getHostConfig()).withDeviceRequests(Collections.singletonList((new DeviceRequest()).withCapabilities(Collections.singletonList(Collections.singletonList("gpu"))).withCount(-1)));
            });
        }
    ...
    protected void containerIsStarted(final InspectContainerResponse containerInfo) {
        if (this.modelName != null) {
            try {
                ...
                ExecResult r = execInContainer(
                        "bash", "-c", launchCmd(this.modelName)
                );
                if (r.getExitCode() != 0) {
