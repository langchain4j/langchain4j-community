...
//    public static final String XINFERENCE_IMAGE = "xprobe/xinference:latest";
    ...
    private static final Map<String, String> MODEL_LAUNCH_MAP = new HashMap<>() {
        {
            put(CHAT_MODEL_NAME, String.format("xinference launch --model-engine Transformers --model-name %s --size-in-billions 0_5 --model-format pytorch --quantization none", CHAT_MODEL_NAME));
            put(GENERATE_MODEL_NAME, String.format("xinference launch --model-engine Transformers --model-name %s --size-in-billions 0_5 --model-format pytorch --quantization none", GENERATE_MODEL_NAME));
            put(VISION_MODEL_NAME, String.format("xinference launch --model-engine Transformers --model-name %s --size-in-billions 2 --model-format pytorch --quantization none", VISION_MODEL_NAME));
            put(RERANK_MODEL_NAME, String.format("xinference launch --model-name %s --model-type rerank", RERANK_MODEL_NAME));
            put(IMAGE_MODEL_NAME, String.format("xinference launch --model-name %s --model-type image", IMAGE_MODEL_NAME));
            put(EMBEDDING_MODEL_NAME, String.format("xinference launch --model-name %s --model-type embedding", EMBEDDING_MODEL_NAME));
        }
    };
